...
@section('content')
      ...
      <form action="/dashboard/posts/{{ $post->slug }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <div class="row">
            <div class="col-6">
              @if ($post->image != null)
                <img src="/storage/{{ $post->image }}" id="preview-image" class="img-thumbnail mb-2">
              @else
                <img src="" id="preview-image" class="img-thumbnail d-none mb-2">
              @endif
            </div>
          </div>
        ...
        <div class="mb-3">
          ...
          <trix-editor input="body" id="editor" required></trix-editor>
          @error('body')
            <div class="invalid-feedback my-0">{{ $message }}</div>
          @enderror
        </div>
  ...
  <script>
    ...
    title.addEventListener('change', function() {
      fetch(`/createSlug?username={{ auth()->user()->username }}&title=${title.value}`)
      .then((response) => {
    ...
    // remove file attachment behaviour in trix editor
